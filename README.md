...
1.1 Как Sun реализовала доступ по индексу к элементу например у нас есть элемент 7 - так как массив хранится у нас линейно в памятит и мы знаем размер блока памити для передоваемого типа данных можем расчитать по формуле (<адресс первого элемента массива> + ((7+1)*<Размер блока>));
