...
13. Как работает gc - там тонкий алгоритм ИИ, который зависит от фазы луны;
14. Чем отличается interface от абстрактного класса - Абстрактный класс может реализовывать методы; интерфейс не может реализовывать методы;
...
по java - что такое type erasure ?
ну еще меня спросил по exception , какие проверяемые не проверяемые. и про protected, public, package уровни доступа
ну это уже классические вопросы, но на них я засыпался чото)
