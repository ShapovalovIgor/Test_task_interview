...
24. Как потоки порождаются - Требуется отнаследоватся от класса Throwable и реализовать метод run или Создать объект класса Thread и реализовать метод run()
...
42. Что включает в себя Synchronizers - различные синхранизаторы потоков;
