...
10.4 Абстракция - придание объекту характеристик, которые чётко определяют его концептуальные границы, отличая от всех других объектов(обращаясь к потомкам класса мы обращаемся к ним как к базовому классу);
...
17.1 Почему у нас исключения поделены на проверяемые/непроверяемые - потому что Uncheked это в основном ошибки программиста он должен был сам например NullPointerException мы же сами можем проверить
...
67. Где хранит JVM данные - в stack(стек) и heap(куча)
